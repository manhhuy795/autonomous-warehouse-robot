...
## Lưu Ý Quan Trọng Khi Push

Thư mục `web/` hiện có `.git` riêng bên trong. Nếu muốn push toàn bộ project như một repo duy nhất, nên xóa `.git` bên trong `web/` trước rồi add lại từ root repo:

```powershell
Remove-Item -Recurse -Force web\.git
git add .
git commit -m "Add robot line system"
git push
```

Nếu muốn giữ `web/` như một submodule, cần tạo `.gitmodules` đúng cách. Với project môn học/NCKH, cách đơn giản nhất thường là dùng một repo duy nhất và không để `.git` lồng trong `web/`.

## Ghi Chú An Toàn

- Không để Wi-Fi thật, mật khẩu thật hoặc IP nội bộ nhạy cảm trong code public nếu repo mở công khai.
- Nếu public repo, nên đổi `WIFI_SSID`, `WIFI_PASSWORD` thành placeholder trước khi push.
- Khi nối UART từ UNO sang ESP32-S3, cần hạ mức tín hiệu TX 5V của UNO xuống 3.3V để bảo vệ ESP32-S3.
- Luôn test motor với tốc độ thấp trước khi chạy robot thật.
